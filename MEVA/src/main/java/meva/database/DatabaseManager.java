...
public class DatabaseManager {
        ...
        public static void initializeDatabase() {
                try (Connection conn = getConnection();
                        ...
                        // [New] 기존 DB 마이그레이션 (컬럼 추가)
                        addColumnsIfNotExist(conn);
         ...
         */
        private static void addColumnsIfNotExist(Connection conn) {
                String[] columns = {
                ...
                try (Statement stmt = conn.createStatement()) {
                        for (String sql : columns) {
                                try {
                                        stmt.execute(sql);
                                        System.out.println("[DB Migration] Added column: " + sql);
                                } catch (SQLException e) {
                                        // Column likely already exists or other error
                                        // SQLite throws error if column exists
                                }
                        }
                } catch (SQLException e) {
                        e.printStackTrace();
